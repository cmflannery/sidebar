...
async fn route(request: HttpRequest) -> HttpResponse {
    if request.method == "GET" {
        ...
        if let Some(turn_id) = request.path.strip_prefix("/api/turns/") {
            if !turn_id.is_empty() {
                return daemon_json(Op::GetTurn {
